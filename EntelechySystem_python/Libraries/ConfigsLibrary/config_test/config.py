...
config = dict(
    神经元总数量=100000,  # 数据类型：整数；配置类别：模型；备注：初始数量。默认 1'000'000。 ；
    运作单元总数量=100000,  # 数据类型：整数；配置类别：模型；备注：初始数量。默认 10'000。 ；
    ...
    is_prerun_config_program=False,  # 数据类型：布尔值；配置类别：操作；备注：是否在实验主程序之前，预先运行了配置库相关的生成配置程序。默认 True。如果要用 False，那么不能用 Python 控制台运行，只能在终端用命令行运行。否则运行的时候不会继续推进。 ；
    is_prerun_agents_program=True,  # 数据类型：布尔值；配置类别：操作；备注：是否在实验主程序之前，预先运行了个体众库相关的生成参数的程序。默认 True； ；
    is_prerun_settings_program=True,  # 数据类型：布尔值；配置类别：操作；备注：是否在实验主程序之前，预先运行了设置库相关的生成参数的程序。默认 True； ；
    is_prerun_parameters_program=True,  # 数据类型：布尔值；配置类别：操作；备注：是否在实验主程序之前，预先运行了参数库相关的生成参数的程序。默认 True； ；
    ...
    is_develop_model=True,  # 数据类型：布尔值；配置类别：开发；备注：是否处于开发模型状态。默认 False； ；
    ...
    engine_version='v0.0.5_alpha',  # 数据类型：字符串；配置类别：配置；备注：引擎版本。 ；
    ...
    program_预加载相关的实验和库文件程序=True,  # 数据类型：布尔值；配置类别：程序；备注：是否运行「预加载库和实验程序」。 ；
