...
config = dict(
    ...
    运作单元总数量=1024*64,  # 数据类型：整数；配置类别：模型；备注：初始数量。默认 10'000。 ；
    单个神经元最大连接数=256,  # 数据类型：整数；配置类别：模型；备注：初始数量。默认 1'000。 ；
    单个运作单元最大连接数=128,  # 数据类型：整数；配置类别：模型；备注：初始数量。默认 1'000。 ；
    ...
    folderpath_models='ComplexIntelligenceSystem/ComplexIntelligenceSystem/Libraries/ModelsLibrary/model_test',  # 数据类型：路径字符串；配置类别：路径；备注：模型所在的文件夹 ；
    folderpath_settings='ComplexIntelligenceSystem/Libraries/SettingsLibrary/settings_test',  # 数据类型：路径字符串；配置类别：路径；备注：设置项所在的文件夹 ；
    folderpath_relpath_ECS='../',  # 数据类型：路径字符串；配置类别：路径；备注：ECS所在工程文件夹相对本实验项目根路径文件夹之相对路径 ；
    folderpath_world_conception_knowledge='ElementalConceptionSystem/ElementalConceptionSystem/Libararies/ConceptionLibrary/conception_test',  # 数据类型：路径字符串；配置类别：路径；备注：世界环境文件夹路径 ；
