...
config = dict(
    计算神经元预留位总数量=2 ** 16,  # 数据类型：整数；配置类别：模型；备注：初始数量。默认 1'000'000。 ；
    ...
    folderpath_engine='EntelechySystem/engine',  # 数据类型：路径字符串；配置类别：路径；备注：模拟器所在工程文件夹名称 ；

    # folderpath_relpath_engine='../',  # 数据类型：路径字符串；配置类别：路径；备注：模拟器所在工程文件夹相对本实验项目根路径文件夹之相对路径 ；  #HACK 2025-02-14：因为合并库内容，所以这个路径不再需要

    folderpath_system='EntelechySystem/Libraries/SystemsLibrary/system_test',  # 数据类型：路径字符串；配置类别：路径；备注：配置项设置所在的文件夹 ；
    folderpath_config='EntelechySystem/Libraries/ConfigsLibrary/config_test',  # 数据类型：路径字符串；配置类别：路径；备注：配置项设置所在的文件夹 ；
    folderpath_parameters='EntelechySystem/Libraries/ParametersLibrary/parameters_test',  # 数据类型：路径字符串；配置类别：路径；备注：参数设置所在的文件夹 ；

    # folderpath_relpath_CIS='../',  # 数据类型：路径字符串；配置类别：路径；备注：CIS所在工程文件夹到本实验项目根路径文件夹之相对路径 ；  #HACK 2024-12-04：因为合并库内容，所以这个路径不再需要

    folderpath_models='EntelechySystem/Libraries/ModelsLibrary/model_111_numpy',  # 数据类型：路径字符串；配置类别：路径；备注：模型所在的文件夹 ；
    # folderpath_models='EntelechySystem/Libraries/ModelsLibrary/model_111_pytorch',  # 数据类型：路径字符串；配置类别：路径；备注：模型所在的文件夹 ；
    # folderpath_models='EntelechySystem/Libraries/ModelsLibrary/model_111_jax',  # 数据类型：路径字符串；配置类别：路径；备注：模型所在的文件夹 ；

    folderpath_settings='EntelechySystem/Libraries/SettingsLibrary/settings_test',  # 数据类型：路径字符串；配置类别：路径；备注：设置项所在的文件夹 ；

    # folderpath_relpath_ECS='../',  # 数据类型：路径字符串；配置类别：路径；备注：ECS所在工程文件夹到本实验项目根路径文件夹之相对路径 ；  #HACK 2024-12-04：因为合并库内容，所以这个路径不再需要

    folderpath_world_conception_knowledge='EntelechySystem/Libararies/ConceptionLibrary/conception_test',  # 数据类型：路径字符串；配置类别：路径；备注：世界环境文件夹路径 ；  #HACK 似乎还不需要

    folderpath_relpath_AWS='../',  # 数据类型：路径字符串；配置类别：路径；备注：AWS所在工程文件夹到本实验项目根路径文件夹之相对路径。除了非 Python、非 JS 的大型世界环境以外，大部分环境已经集成在该项目中；

    folderpath_world_environment='EntelechySystem/Libraries/WorldLibrary/Virtual2DMiniNurseryEnv',  # 数据类型：路径字符串；配置类别：路径；备注：世界环境文件夹路径 ；
    # folderpath_agents='EntelechySystem/Libraries/AgentsLibrary/agents_test',  # 数据类型：路径字符串；配置类别：路径；备注：模型所在的文件夹 ； #HACK 似乎还不需要
